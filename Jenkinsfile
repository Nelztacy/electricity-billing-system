pipeline {
	...
	stage ('SAST') {
       ...
       stage ('Deploy-To-Tomcat') {
            steps {
           sshagent(['tomcat']) {
                sh 'scp -o StrictHostKeyChecking=no target/*.war tomcat@10.0.0.101:/home/tomcat/usr/libexec/tomcat9/webapps/webapp.war'
              }     
           }      
    }
	    ...
	    stage ('DAST') {
		    	...
		    	steps {
			    sshagent(['zap']) {
				    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.85.77.1 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://10.0.0.101:8080/webapp/" || true'
			    }
			}
		}   
	    ...
	    stage ('Nikto Scan') {
			...
			sh 'docker run --user $(id -u):$(id -g) --rm -v $(pwd):/report -i secfigo/nikto:latest -h 10.0.0.101 -p 8080 -output /report/nikto-output.xml'
	    ...
	    stage ('SSL Checks') {
			...
			sh 'python -m sslyze --regular 10.0.0.101:8080 --json_out sslyze-output.json'
